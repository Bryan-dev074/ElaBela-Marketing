...
import { Eye, EyeOff, Users, Lock, Plus, Copy, Check, Pencil, Trash2, Globe } from "lucide-react";
...
const seed: Cred[] = CREDENTIAL_PLATFORMS.map((c, i) => ({
  id: "c" + i, platform: c.platform, icon: c.icon, idType: "email", identifier: "", secret: "", scope: c.scope as "shared" | "private",
}));
    ...
    <div className="group flex items-center gap-3 rounded-xl border border-white/8 bg-black/20 px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {configured ? (
          <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 font-mono text-xs text-[var(--muted)]">
            <span className="capitalize text-[var(--faint)]">{c.idType}:</span><span>{show ? c.identifier || "—" : "•".repeat(Math.min(10, (c.identifier || "······").length))}</span>
            <span className="text-[var(--faint)]">clave:</span><span>{show ? c.secret || "—" : "••••••••"}</span>
          </div>
      ...
      <div className="flex shrink-0 items-center gap-1">
        {configured && <>
          ...
          <button onClick={() => copy(c.secret, c.id)} className="rounded-lg border border-white/10 p-1.5 text-[var(--faint)] transition hover:text-white" aria-label="Copiar">{copied === c.id ? <Check className="h-3.5 w-3.5 text-emerald-400" /> : <Copy className="h-3.5 w-3.5" />}</button>
        </>}
...
export default function CredencialesView({ role }: { role: Role }) {
  ...
  const priv = creds.filter((c) => c.scope === "private");
  ...
  return (
    <div>
      ...
      <Card className="mb-6 flex items-start gap-3 border-amber-400/20 bg-amber-400/5 p-4" hover={false}>
        ...
        <p className="text-xs text-amber-200/90">Los valores se cifran en producción y nunca se guardan en el repositorio. Cargalos acá, mostralos con 👁 y copialos.</p>
      </Card>
      ...
      <Card className="mb-6 p-6" hover={false}>
        <div className="mb-1 flex items-center gap-2">
          ...
          <span className="rounded-full border border-emerald-400/30 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium text-emerald-300">Todos las ven</span>
        </div>
        <p className="mb-4 text-xs text-[var(--muted)]">Cuentas y herramientas que usa todo el equipo.</p>
        <div className="space-y-2.5">{shared.map((c) => <Row key={c.id} c={c} onEdit={() => setEditing(c)} onDelete={() => remove(c.id)} />)}</div>
      </Card>
      ...
      <Card className="p-6" hover={false}>
        <div className="mb-1 flex items-center gap-2">
          ...
          <span className="rounded-full border border-nude/30 bg-nude/10 px-2 py-0.5 text-[10px] font-medium text-nude">Solo vos</span>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          {priv.length === 0 && <div className="rounded-xl border border-dashed border-white/10 py-6 text-center text-sm text-[var(--muted)]">Sin credenciales privadas. Agregá una con «Agregar acceso».</div>}
        </div>
      </Card>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="inline-flex rounded-xl border border-white/10 bg-white/5 p-1">
                <button type="button" onClick={() => setEditing({ ...editing, scope: "shared" })} className={`flex items-center gap-1.5 rounded-lg px-3.5 py-1.5 text-xs font-medium transition ${editing.scope === "shared" ? "bg-white text-black" : "text-[var(--muted)]"}`}><Globe className="h-3.5 w-3.5" /> Compartida</button>
                <button type="button" onClick={() => setEditing({ ...editing, scope: "private" })} className={`flex items-center gap-1.5 rounded-lg px-3.5 py-1.5 text-xs font-medium transition ${editing.scope === "private" ? "bg-white text-black" : "text-[var(--muted)]"}`}><Lock className="h-3.5 w-3.5" /> Privada</button>
              </div>
            </div>
