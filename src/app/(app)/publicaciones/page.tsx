...
import { CheckCircle2, AlertTriangle, XCircle, Plus, Pencil, ImagePlus } from "lucide-react";
...
export default function PublicacionesPage() {
  ...
  function save() {
    ...
    setTypes((prev) => (editing.id ? prev.map((t) => (t.id === editing.id ? editing : t)) : [...prev, { ...editing, id: "p" + Date.now() }]));
    setEditing(null);
  }
          ...
          <Reveal key={p.id} delay={i * 0.04}>
            <Card className="group flex h-full flex-col overflow-hidden p-0">
              {p.exampleImage && (
                ...
                <img src={p.exampleImage} alt="" className="h-36 w-full object-cover" />
              )}
              <div className="flex flex-1 flex-col p-5">
                ...
                {p.example && <div className="mt-3 rounded-xl border border-white/8 bg-black/20 p-3"><p className="mb-1 text-[10px] uppercase tracking-wider text-[var(--faint)]">Ejemplo</p><p className="text-xs text-[var(--muted)]">{p.example}</p></div>}
              </div>
            </Card>
          </Reveal>
        ))}
              ...
              {WEEKLY_REQS.map((r, i) => {
                ...
                return (
                  <motion.tr key={r.platform + r.format} initial={{ opacity: 0, x: -8 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} className="border-b border-white/6 last:border-0">
                    ...
                    <td className="px-5 py-4"><div className="flex items-center gap-2"><div className="h-1.5 w-24 overflow-hidden rounded-full bg-white/10"><div className={`h-full rounded-full ${pct >= 100 ? "bg-emerald-400" : pct >= 60 ? "bg-blue-400" : "bg-amber-400"}`} style={{ width: `${pct}%` }} /></div><span className="text-xs text-[var(--faint)]">{r.done}/{r.target}</span></div></td>
            ...
            <Field label="Imagen de ejemplo (propia)">
              <div className="flex items-center gap-3">
                {editing.exampleImage && (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={editing.exampleImage} alt="" className="h-16 w-24 rounded-lg object-cover" />
                )}
                ...
                {editing.exampleImage && <button type="button" onClick={() => setEditing({ ...editing, exampleImage: "" })} className="text-xs text-[var(--faint)] hover:text-red-300">Quitar</button>}
              </div>
            </Field>
          </div>
        )}
