...
-- VERIFICATION: devuelve una sola columna booleana y no lee datos de usuarios.
select (
  (select count(*) = 16
   from (values
     ('profiles', 'avatar'),
     ('daily_tasks', 'day_assignees'),
     ('daily_tasks', 'post_type'),
     ('projects', 'responsible_usernames'),
     ('projects', 'project_type'),
     ('projects', 'priority'),
     ('projects', 'completed_by'),
     ('post_types', 'guide'),
     ('post_types', 'example_images'),
     ('story_config', 'done_date'),
     ('tool_items', 'category_id'),
     ('brand_assets', 'storage_path'),
     ('credentials', 'category_id'),
     ('credential_categories', 'scope'),
     ('daily_task_logs', 'completed_by'),
     ('daily_task_logs', 'activity_date')
  ...
  and to_regclass('public.tool_categories') is not null
  and to_regclass('public.credential_categories') is not null
  and to_regclass('public.daily_task_logs') is not null
  and to_regclass('public.projects_completed_at_idx') is not null
  and (select count(*) = 3
       from pg_class c
       join pg_namespace n on n.oid = c.relnamespace
       where n.nspname = 'public'
         and c.relname in ('tool_categories', 'credential_categories', 'daily_task_logs')
         and c.relrowsecurity)
  and exists (
    select 1 from storage.buckets
    where id = 'elabela-assets'
      and name = 'elabela-assets'
      and public
  ...
  and (select count(*) = 17
       ...
       where (schemaname, tablename, policyname) in (
         ('public', 'tool_categories', 'tool_categories_all'),
         ('public', 'credential_categories', 'credential_categories_select'),
         ('public', 'credential_categories', 'credential_categories_insert'),
         ('public', 'credential_categories', 'credential_categories_update'),
         ('public', 'credential_categories', 'credential_categories_delete'),
         ('public', 'credentials', 'credentials_select'),
         ('public', 'credentials', 'credentials_insert'),
         ('public', 'credentials', 'credentials_update'),
         ('public', 'credentials', 'credentials_delete'),
         ('public', 'daily_task_logs', 'daily_task_logs_select'),
         ('public', 'daily_task_logs', 'daily_task_logs_insert'),
         ('public', 'daily_task_logs', 'daily_task_logs_update'),
         ('public', 'daily_task_logs', 'daily_task_logs_delete'),
         ('storage', 'objects', 'elabela_assets_insert'),
         ('storage', 'objects', 'elabela_assets_select'),
         ('storage', 'objects', 'elabela_assets_update'),
         ('storage', 'objects', 'elabela_assets_delete')
       ))
  and has_function_privilege('authenticated', 'public.move_and_delete_tool_category(text,text)', 'EXECUTE')
  and has_function_privilege('authenticated', 'public.reorder_tool_categories(text[])', 'EXECUTE')
  and has_function_privilege('authenticated', 'public.delete_empty_credential_category(text)', 'EXECUTE')
  and has_function_privilege('authenticated', 'public.reorder_credential_categories(text,text[])', 'EXECUTE')
) as ok;
