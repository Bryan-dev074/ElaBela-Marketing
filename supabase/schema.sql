...
create policy projects_all on public.projects for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
...
create policy guiones_all on public.guiones for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
...
create policy post_types_all on public.post_types for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
...
create policy brand_assets_all on public.brand_assets for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
...
create policy calendar_events_all on public.calendar_events for all using (auth.role() = 'authenticated') with check (auth.role() = 'authenticated');
...
create policy credentials_insert on public.credentials for insert
  with check (auth.role() = 'authenticated' and (scope = 'shared' or owner_id = auth.uid() or owner_id is null));
drop policy if exists credentials_update on public.credentials;
create policy credentials_update on public.credentials for update
  using (scope = 'shared' or owner_id = auth.uid() or owner_id is null)
